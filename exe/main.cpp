...
bool ejectProcess(HANDLE hProcess, const HMODULE dllHandle) {
    bool bOk = false; // Assume that the function fails
    HANDLE hThread = nullptr;
    ...
    do {
        ...
        HANDLE hThread = CreateRemoteThread(hProcess, nullptr, 0,
            pfnThreadRtn, (LPVOID)dllHandle, 0, nullptr);
        if (hThread == nullptr) break;
...
int wmain() {
    ...
    for (const auto& pid : pids) {
        ...
        if (injected.value() != nullptr) {
            wcout << "Found injected handle at " << injected.value() << endl;
            ejectProcess(process, injected.value());
